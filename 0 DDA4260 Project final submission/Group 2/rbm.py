...
def ratingsPerMovie(training):
    ...
    return np.array(
        [
            [i, movie, len([x for x in training if x[0] == movie])]
            for i, movie in enumerate(u_movies)
        ]
    )
...
def visibleToHiddenVec(v, w, b):
    ...
    # m = v.shape[0]  # number of visible unit
    # F = w[0].shape[1]  # number of hidden units
    m, F, K = w.shape
    ret = np.zeros((F))
    for j in range(F):
        sum = 0
...
def hiddenToVisible(h, w, b):
    ...
    for i in range(m):
        for k in range(K):
            ...
            # h is matrix,h[j]is a vector
            ret[i, k] = sum + b[i, k]  # this is for the bias step
...
def getPredictedDistribution(v, w, wq, b, b1):
    ### TO IMPLEMENT ###
    ...
    hidden_vec = visibleToHiddenVec(v, w, b)
    sample_hidden_vec = sample(hidden_vec)
    F, K = wq.shape
    ret = np.zeros(K)
    for k in range(K):
        for j in range(F):
            ret[k] += wq[j, k] * sample_hidden_vec[j]

        ret[k] += b1[k]
    return softmax(ret)

    # num_of_hidden_node = visibleToHiddenVec(v, w)
    # sample_the_state = sample(num_of_hidden_node)  # size F x 1
    # # ret = np.transpose(
    # #     np.dot(np.transpose(sample_the_state), wq)
    # # )  # 1 x F * F x 5 -> 1 x 5

    # ret = np.transpose(np.dot(np.transpose(sample_the_state), wq))
    # return ret
...
def predictRatingMax(ratingDistribution):
    ...
    max_value = np.max(ratingDistribution)
    ret = np.argmax(ratingDistribution)
    return ret
...
def predictRatingExp(ratingDistribution):
    ...
    ascending_vector = np.arange(
        1, K + 1
    )  # here I create a vector 5 x 1, like[1,2,3,4,5]

    ret = np.dot(
        np.transpose(ratingDistribution), ascending_vector
    )  # 1 x 5 * 5 x 1 -> 1
    return ret


def predictMovieForUser(q, user, W, allUsersRatings, b, b1, predictType="exp"):
    # movie is movie idx
    # user is user ID
    ...
    ratingDistribution = getPredictedDistribution(
        v, W[ratingsForUser[:, 0], :, :], W[q, :, :], b, b1[q, :]
    )
    if predictType == "max":
...
def predict(movies, users, W, allUsersRatings, b, b1, predictType="exp"):
    # given a list of movies and users, predict the rating for each (movie, user) pair
    # used to compute RMSE
    return [
        predictMovieForUser(
            movie, user, W, allUsersRatings, b, b1, predictType=predictType
        )
        for (movie, user) in zip(movies, users)
    ]


def predictForUser(user, W, allUsersRatings, b, b1, predictType="exp"):
    ### TO IMPLEMENT
    # given a user ID, predicts all movie ratings for the user
    number_of_movies = W.shape[0]
    ret = np.zeros((number_of_movies))
    for q in range(number_of_movies):
        ret[q] = predictMovieForUser(q, user, W, allUsersRatings, b, b1, predictType)
    return ret
